...
static token numeric_case(token_stream tstream_ptr,token token_ptr,char ch,char token_str[],int token_ind);  //false 將空白處補上(token_stream tstream_ptr,token token_ptr,char ch,char token_str[],int token_ind)
static token error_or_eof_case(token_stream tstream_ptr,token token_ptr,int cu_state,char token_str[],int token_ind,char ch); //false 將空白處補上(token_stream tstream_ptr,token token_ptr,int cu_state,char token_str[],int token_ind,char ch)
static int check_delimiter(char ch);//false 將空白處補上(char ch)
...
static int next_state(int state , char ch);//false 將空白處補上(int state , char ch)
static get_actual_token(int token_ind , char token_str[]);//false 將()內原本的東西改成(int token_ind , char token_str[])
...
character_stream stream_ptr;
{
      if(stream_ptr->stream[stream_ptr->stream_ind] == EOF && TRUE)
            return(TRUE)
      else
            return(FALSE);
}
 ...
 * ****************************************************************** */
//false 下面的(void)->(FILENAME)
token_stream open_token_stream(FILENAME)
sting FILENAME //false 補上宣告此變數string FILENAME
{
	  ...
	  if (next_st == -1) { /* ERROR or EOF case */
	      ...
	      return(numeric_case(tstream_ptr,token_ptr,ch,
    	    ...
    	    if(!token_found)
    	    {
    	      ...
    	      token_ind--;
    	      get_actual_token(token_str,token_ind);//false 將()內缺少的值補上(token_str,token_ind)
	  ...
	  switch(next_st) 
            { 
                 ...
                 case 29 : token_ptr->token_id=constant(next_st,token_str,token_ind);
                           get_actual_token(token_str,token_ind);//false 將()內缺少的值補上(token_str,token_ind)
                           //false 將get_actual_token(token_str);這行多餘的刪掉掉
                           strcpy(token_ptr->token_string,token_str);
                           return(token_ptr);
 ...
 * ***************************************************************** */
//false 下面的 keyword 型別應為 int 非 double
